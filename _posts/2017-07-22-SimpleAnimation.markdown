...
如前面章节 [Scenes and Nodes](gltfTutorial_004_ScenesNodes.md)所述，每个节点可以有一个局部变换。这个变换可以由节点的 `matrix` 属性给出，或者是 `translation`，`rotation`和`scale`(TRS)属性。
...
以下是先前展示的 [最小glTF文件](gltfTutorial_003_MinimalGltfFile.md)，但是扩展了动画。本节将说明添加此动画所做的更改和扩展。
...
glTF中添加了一个新的ID为`"buffer1"`的`buffer`，。 此缓冲区同样使用[data URI](gltfTutorial_002_BasicGltfStructure.md#binary-data-in-data-uris)来编码100个字节的动画数据：
...
关于内插计算的更详细的描述和实际示例，可以参考[Animations](gltfTutorial_006_Animations.md)部分。
